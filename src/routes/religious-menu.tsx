...
import heroImg from "@/assets/site/menu-2.jpeg";
...
type TabKey = "STARTERS" | "MAINS COURSE" | "DESSERTS" | "BEVERAGES";
...
const TABS: TabKey[] = ["STARTERS", "MAINS COURSE", "DESSERTS", "BEVERAGES"];

export const Route = createFileRoute("/religious-menu")({
  head: () => ({
    meta: [
      { title: "Religious Menu— Al Najaf Catering" },
      {
        name: "description",
        content:
          "Discover Al Najaf's luxury Religious Menu— starters, mains, desserts and beverages crafted for unforgettable celebrations.",
      },
    ],
  }),
  component: ReligiousMenuPage,
});
    ...
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
      ...
      <section className="relative h-[70vh] min-h-[460px] md:h-[560px] w-full overflow-hidden">
        <div className="absolute inset-0">
          <img
            src={heroImg}
            alt="Elegant religious table setup"
        ...
        <div className="relative z-10 h-full flex flex-col items-center justify-center px-6 text-center text-white">
          ...
          >
            {["HOME", "MENUS", "RELIGIOUS MENU"].map((c, i, arr) => (
              <motion.span
                key={c}
                variants={{
              ...
              >
                <span className={i === arr.length - 1 ? "text-gold" : ""}>{c}</span>

                {i < arr.length - 1 && <ChevronRight className="w-3 h-3 text-gold" />}
              </motion.span>
            ))}
          </motion.nav>
        </div>
